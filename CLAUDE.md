...
**שורות קבוצתיות לא-מאוישות מוסתרות בלוח (`rosterBoardHtml`):** מתגבר, PF, נחים PF, PMS, PMS נחים, מילואים, תורנות — כל אחת מוצגת רק אם יש בה שם אמיתי באחד מימי השבוע (`hasAny()`). שורות שלד (מנהל/ר״צ/ל-ר״צ/מטיס/נהג/כלים) ותורן טייסת מוצגות תמיד.
...
**השבתה ידנית של שורה (`roster.disabledRows`):** בעורך הלוח, הקשה על מתג ה-on/off (`.rblk-toggle-btn` — מתג track+כדור אמיתי, לא אייקון עין/אמוג'י) ליד כותרת שורה קבוצתית (מתגבר/PF/נחים PF/PMS/PMS נחים/מילואים/תורנות, וגם שורות מותאמות-אישית) מסמנת אותה "מושבתת". שורה מושבתת לא מוצגת בלוח **בשום מצב**, גם אם יש בה שיבוץ בפועל — שונה מ"לא מאוישת" שמתעדכן אוטומטית לפי הנתונים.
...
- `roster_custom_rows_test.mjs` — שורות מותאמות-אישית בלוח (הוספה/עריכה/מיקום/הסתרה כשריקות)
