...
⚠️ **התנגשות גרסאות בין סשנים מקבילים:** לפני שמעלים את המספר, לבדוק
תמיד את `CACHE_NAME` בפועל ב-`origin/main` העדכני (`git fetch origin main`)
— לא להסתמך על הזיכרון/היסטוריית הענף המקומי. סשן מקביל אחר יכול כבר
להעלות את המספר בינתיים מסיבה לא־קשורה; אם שני סשנים "מעלים ב-1" באופן
עיוור מאותה נקודת מוצא ישנה, שניהם עלולים לנחות על אותו מספר גרסה. אם
זה קורה, הקובץ אחרי המיזוג יוצא **זהה בייטים** למה שכבר פרוס בפועל —
הדפדפן לא מזהה שום עדכון, הבאנר "גרסה חדשה זמינה" לא מופיע בכלל, וכל
השינויים (כולל קוד לא-קשור לגמרי) נשארים תקועים בקאש הישן בלי אף אזהרה.
זה בדיוק מה שקרה ב-2026-08-19 (התנגשות על v52) — לבדוק תמיד לפני דחיפה.
