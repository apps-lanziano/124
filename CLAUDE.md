...
**תיקון-עצמי רטרואקטיבי (`healLegacyLeakedCustomRows`):** הזריעה למעלה חלה רק על שורות שנוצרות **מרגע התיקון והלאה** — שורות מותאמות-אישית שכבר דלפו *לפני* שהזריעה נוספה (קיימות ב-production, מאוישות בפועל בחלק מהשבועות בלבד) לא תוקנו רטרואקטיבית על ידה. `healLegacyLeakedCustomRows()` סוגר את הפער: רץ אוטומטית פעם אחת בסשן מתוך `renderRosterView()` (בלי צורך בפעולה ידנית של המ״ע), בודק לכל שורה מותאמת-אישית אם יש לה שיבוץ בפועל בחלק מ-current/next/prev אך לא בכולם — ואם כן, מסמן אותה מושבתת-מראש בשבועות שבהם היא ריקה. לא נוגע בשורה שריקה **בכל** השבועות (אין איתות למה היא שייכת — נשארת לזריעה העתידית/למתג הידני) ולא בשורה שמשובצת **בכל** השבועות (אין מה לתקן). אידמפוטנטי לחלוטין (בודק `disabledRows` לפני כתיבה) — בטוח שירוץ שוב ושוב, פשוט הופך ל-no-op אחרי הריצה הראשונה שבאמת תיקנה משהו.
