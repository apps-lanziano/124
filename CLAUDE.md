...
**שורות מותאמות-אישית:** מ״ע תורנויות יכול להוסיף שורות נוספות ללוח דרך "שורות מותאמות אישית" (`openRosterCustomRowsMgr`) — נגיש **רק** מתוך עורך הלוח עצמו (`renderRosterEditor`, כפתור "➕ הוסף שורה מותאמת אישית ללוח"), במכוון לא כבאנר נפרד במסך הראשי. הגדרות (`{id, label, afterKey}`) נשמרות גלובלית תחת `roster_custom_rows` (לא לכל שבוע), והשיוך בפועל חי בכל יום תחת `custom_<id>` (מערך שמות, כמו כל שדה רשימה אחר). `afterKey` קובע מיקום ("__start__" / מפתח שורה קבועה / id של שורה אחרת) — `buildRosterRowOrder()` פותר את הסדר הסופי.
