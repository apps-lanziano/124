...
**שורות מותאמות-אישית:** מ״ע תורנויות יכול להוסיף שורות נוספות ללוח דרך "שורות מותאמות אישית" (`openRosterCustomRowsMgr`) — נגיש גם מהבאנר הראשי וגם מכפתור בתוך עורך הלוח עצמו (`renderRosterEditor`). הגדרות (`{id, label, afterKey}`) נשמרות גלובלית תחת `roster_custom_rows` (לא לכל שבוע), והשיוך בפועל חי בכל יום תחת `custom_<id>` (מערך שמות, כמו כל שדה רשימה אחר). `afterKey` קובע מיקום ("__start__" / מפתח שורה קבועה / id של שורה אחרת) — `buildRosterRowOrder()` פותר את הסדר הסופי. תחומות ללשונית "לוח שבועי" בלבד — לא מוזרמות ל"רק אני"/"לוח יומי"/`rosterMyRoles`/דוח החריגות/התראות.
