...
1 מופעים (שורה 3956). מפתח Web של Firebase הוא ציבורי מעצם טיבו — ההגנה בפועל היא כללי מסד הנתונים + App Check. לא נדרשת פעולה, בתנאי ששני אלה מופעלים.
...
1195 KB, 17,207 שורות בקובץ יחיד. מעל 900KB — כל טעינה ראשונה מורידה את הכל. שווה לשקול פיצול ה-CSS/JS לקבצים נפרדים שנשמרים במטמון בנפרד.
...
4 פונקציות מעל 120 שורות. הארוכות: renderRosterEditor (148 שורות, שורה 9096), applyLoginUiForRole (146 שורות, שורה 4887), renderRosterView (128 שורות, שורה 7321). פיצול יקל על תחזוקה ויקטין סיכון לבאגים.
...
1. **גודל האפליקציה** — 1195 KB, 17,207 שורות בקובץ יחיד.
