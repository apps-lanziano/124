...
**1. בדיקה נכשלה: constraint_edit_resubmit_and_daily_wa_test.mjs**
❌ אישור מ״ע הישן נוקה בעריכה - false
...
- הרצתי **109 בדיקות** שמוודאות שתקלות שכבר תוקנו לא חזרו
...
**🔴 חמור · בדיקה נכשלה: constraint_edit_resubmit_and_daily_wa_test.mjs**
❌ אישור מ״ע הישן נוקה בעריכה - false
...
1 מופעים (שורה 3729). מפתח Web של Firebase הוא ציבורי מעצם טיבו — ההגנה בפועל היא כללי מסד הנתונים + App Check. לא נדרשת פעולה, בתנאי ששני אלה מופעלים.
...
967 KB, 15,843 שורות בקובץ יחיד. מעל 900KB — כל טעינה ראשונה מורידה את הכל. שווה לשקול פיצול ה-CSS/JS לקבצים נפרדים שנשמרים במטמון בנפרד.
...
1 פונקציות מעל 120 שורות. הארוכות: applyLoginUiForRole (123 שורות, שורה 4655). פיצול יקל על תחזוקה ויקטין סיכון לבאגים.
...
1. **בדיקה נכשלה: constraint_edit_resubmit_and_daily_wa_test.mjs** — ❌ אישור מ״ע הישן נוקה בעריכה - false.
2. **גודל האפליקציה** — 967 KB, 15,843 שורות בקובץ יחיד.
